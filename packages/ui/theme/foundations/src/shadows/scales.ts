import type { ShadowScale } from "./types";

// Base shadow scale
export const baseShadowScale: ShadowScale = {
	none: "0 0 #0000",
	sm: "0 1px 2px 0 rgb(0 0 0 / 0.05)",
	base: "0 1px 3px 0 rgb(0 0 0 / 0.1), 0 1px 2px -1px rgb(0 0 0 / 0.1)",
	md: "0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)",
	lg: "0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)",
	xl: "0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)",
	"2xl": "0 25px 50px -12px rgb(0 0 0 / 0.25)",
	inner: "inset 0 2px 4px 0 rgb(0 0 0 / 0.05)",
};

// Soft shadow scale (more subtle)
export const softShadowScale: ShadowScale = {
	none: "0 0 #0000",
	sm: "0 1px 2px 0 rgb(0 0 0 / 0.03)",
	base: "0 1px 3px 0 rgb(0 0 0 / 0.05), 0 1px 2px -1px rgb(0 0 0 / 0.05)",
	md: "0 4px 6px -1px rgb(0 0 0 / 0.05), 0 2px 4px -2px rgb(0 0 0 / 0.05)",
	lg: "0 10px 15px -3px rgb(0 0 0 / 0.05), 0 4px 6px -4px rgb(0 0 0 / 0.05)",
	xl: "0 20px 25px -5px rgb(0 0 0 / 0.05), 0 8px 10px -6px rgb(0 0 0 / 0.05)",
	"2xl": "0 25px 50px -12px rgb(0 0 0 / 0.15)",
	inner: "inset 0 2px 4px 0 rgb(0 0 0 / 0.03)",
};

// Strong shadow scale (more pronounced)
export const strongShadowScale: ShadowScale = {
	none: "0 0 #0000",
	sm: "0 1px 2px 0 rgb(0 0 0 / 0.1)",
	base: "0 1px 3px 0 rgb(0 0 0 / 0.15), 0 1px 2px -1px rgb(0 0 0 / 0.15)",
	md: "0 4px 6px -1px rgb(0 0 0 / 0.15), 0 2px 4px -2px rgb(0 0 0 / 0.15)",
	lg: "0 10px 15px -3px rgb(0 0 0 / 0.15), 0 4px 6px -4px rgb(0 0 0 / 0.15)",
	xl: "0 20px 25px -5px rgb(0 0 0 / 0.15), 0 8px 10px -6px rgb(0 0 0 / 0.15)",
	"2xl": "0 25px 50px -12px rgb(0 0 0 / 0.3)",
	inner: "inset 0 2px 4px 0 rgb(0 0 0 / 0.1)",
};

export const shadowScales = {
	base: baseShadowScale,
	soft: softShadowScale,
	strong: strongShadowScale,
} as const;

export type ShadowScaleName = keyof typeof shadowScales;
